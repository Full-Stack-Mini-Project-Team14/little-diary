...
def check_dup():
    ...
    print(exists)
    if exists:
        return jsonify({'result': 'fail', 'msg': 'nono '})
    else :
        return jsonify({'result': 'true', 'msg': 'yes'})
...
#         payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
#         print
#         userinfo = db.user.find_one({ 'id_receive' : payload[id]}, {'_id': 0})
#     return jsonify({'result': 'success', 'id': userinfo['nick']})
#
#     except jwt.ExpiredSignatureError:
